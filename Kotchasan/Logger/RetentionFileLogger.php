...
/**
 * File logger that trims entries older than a retention period.
 *
 * Cleanup runs at most once per $cleanupInterval seconds, tracked by a hidden
 * marker file beside the log, so a busy request never pays for a rewrite.
 *
 * @package Kotchasan\Logger
 */
class RetentionFileLogger extends FileLogger
{
    /**
    ...
    /**
     * Create the log directory when it does not exist yet.
     *
     * @param string $logFile
     *
     * @return void
     */
    protected function ensureLogDirectory(string $logFile): void
    {
        $dir = dirname($logFile);
        if (!is_dir($dir)) {
            @mkdir($dir, 0755, true);
    ...
    protected function shouldKeepLine(string $line, int $cutoff): bool
    {
        ...
        if (preg_match('/^\[([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2})\]/', $line, $match)) {
            ...
            return ($timestamp === false || $timestamp >= $cutoff) && $this->isRetainableLine($line);
        }
    ...
    /**
     * Check whether a timestamped line belongs to this log.
     * Subclasses narrow this to drop foreign entries during cleanup.
     *
     * @param string $line
     *
     * @return bool
     */
    protected function isRetainableLine(string $line): bool
    {
        return true;
    }
