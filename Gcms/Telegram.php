...
class Telegram extends \Kotchasan\KBase
{
     ...
     *
     * Telegram keeps the button spinning in the user's client until this is
     * called — up to about 30 seconds. The work behind the button usually
     * finishes in milliseconds, so without this the interface looks broken
     * precisely when it worked.
     *
     * @param string      $callbackQueryId `callback_query.id` from the update
     * @param string      $text            short toast shown to the user
     * @param string|null $botToken
     *
     * @return string empty on success, otherwise the error
     */
    public static function answerCallbackQuery($callbackQueryId, $text = '', $botToken = null)
    {
        ...
        if ($text !== '') {
            $params['text'] = mb_substr(self::toText($text), 0, 200);
        }

        $response = self::sendRequest('answerCallbackQuery', $params, $botToken);
        if ($response === false) {
        ...
        return isset($response['ok']) && !$response['ok']
            ? (string) ($response['description'] ?? 'Telegram API error')
            : '';
    }
