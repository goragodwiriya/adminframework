...
`cli-sync-core.php` ตัดสินต่อไฟล์: เหมือน/แค่เก่า (ตรงกับรุ่นก่อนหรือกับโปรเจ็คอื่น) → ทับ ·
โปรเจ็คแก้เฉพาะตัว → 3-way merge แล้วรายงานว่าต้อง backport · merge ไม่ได้ → ไม่แตะ ·
ของเดิมทุกไฟล์ที่ถูกทับอยู่ที่ `datas/core-sync-backup/<เวลา>/` ของโปรเจ็คนั้น
โปรเจ็คตระกูล GCMS (มี `admin/`) ได้รับเฉพาะ `Kotchasan/` + `Now/`
