...
**0. สิ่งที่แกนทำให้แล้ว (7.0.4)**

| เรื่อง | ที่ | หมายเหตุ |
|---|---|---|
| รหัสผ่านผู้ใช้ | `Kotchasan\Password::hash() / verify() / needsRehash()` | bcrypt ของ HMAC(`password_key`, รหัสผ่าน) · hash sha1 รุ่นเก่ายังล็อกอินได้และถูกเก็บใหม่ทันทีที่ล็อกอิน · **ห้ามเรียก sha1() เก็บรหัสผ่านเองอีก** ใช้ `\Index\Auth\Model::hashPassword()` |
| session ที่ API เปิด | `Kotchasan\Http\Request::startSecureSession()` | HttpOnly · SameSite=Lax · Secure บน HTTPS · use_strict_mode — ทุก `session_start()` ในแกนผ่านที่นี่ |
| IP ของผู้ใช้ | `$request->getClientIp()` | เชื่อ `X-Forwarded-For` เฉพาะเมื่อคำขอมาจาก IP ใน `TRUSTED_PROXIES` (`load.php`) — ตั้งเมื่ออยู่หลัง reverse proxy ไม่งั้น rate limit / `api_ips` ถูกหลบด้วย header ปลอม |
| บัญชีถูกระงับ | `Index\Auth\Model::getUserByToken()` | `active = 0` → token เดิมใช้ไม่ได้ทันที และการระงับ (ตาราง/ฟอร์มสมาชิก) ลบ session ทุกเครื่อง |
| หน้า HTML | `index.php` | `X-Frame-Options: SAMEORIGIN` · `frame-ancestors 'self'` · `nosniff` · `Referrer-Policy` (API มีของตัวเองใน `Kotchasan\Http\Response`) |
| ตาราง | Now.js TableManager | ค่าจากแถวไม่ถูกตีความเป็น template/`{LNG_}` อีก · `data-if` ในเซลล์ทำงานต่อแถว · เซลล์ข้อมูลมี `translate="no"` |
| ข้อมูลผู้ใช้ที่มี `{LNG_…}` | `translate="no"` | ใส่ที่ element ที่แสดงข้อมูลผู้ใช้ (`data-text`, `data-for`) — I18nManager จะไม่แปลข้างใน |

ตรวจซ้ำได้ด้วย `php install/cli-testsecurity.php --url=<โดเมน>` และ `nowjs/tests/table-cell-data-safety.mjs`
