...
class Controller extends ApiController
{
    ...
    private static function hideValues(array $data)
    {
        foreach (self::HIDDEN_VALUES as $key) {
            if (array_key_exists($key, $data)) {
                ...
                $data[$key] = '';
                $data[$key.'_placeholder'] = $configured ? '{LNG_Leave blank to keep existing}' : null;
            }
        }
    ...
    private function getCompanyData()
    {
        ...
        // Company signature — used on printed documents together with the stamp
        if (file_exists(ROOT_PATH.DATA_FOLDER.'images/company_signature'.self::$cfg->stored_img_type)) {
            $result['company_signature'] = [
                [
                    'url' => WEB_URL.DATA_FOLDER.'images/company_signature'.self::$cfg->stored_img_type,
     ...
     * @return mixed
     */
    public function removeCompanySignature(Request $request)
    {
        return $this->removeImage($request, 'company_signature');
    }
    ...
    private function parseCompanySettings($body, $config)
    {
        ...
        $config->company['authorized'] = Text::topic($body['company_authorized'] ?? '');
    ...
    private function imageUpload(Request $request)
    {
        ...
        foreach ($request->getUploadedFiles() as $item => $file) {
            if (in_array($item, ['logo', 'bg_image', 'company_logo', 'company_stamp', 'company_signature', 'site_logo', 'pwa_icon', 'screenshot'])) {
    ...
    private function removeImage(Request $request, $item)
    {
        try {
            ...
            $allowedItems = ['logo', 'bg_image', 'company_logo', 'company_stamp', 'company_signature', 'site_logo', 'pwa_icon', 'screenshot'];
